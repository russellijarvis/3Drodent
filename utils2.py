...
class Utils(HocUtils):
    ...
    def generate_cells(self):
        ...
        for cell_data in self.cells_data:
            ...
            #No don't get it from the JSON manifest, manifests are too hard to automatically create atm.
            swclist=glob.glob('*.swc')
            # Filter out the bad morphology, using a list comprehension.
            swclist2=[i for i in swclist if "Scnn1a-Tg3-Cre_Ai14_IVSCC_-177300.01.02.01_473845048_m.swc" not in i] 
            swclist2=[i for i in swclist if "466664172.swc" not in i] 

            #swclist2=[i for i in swclist: if "Scnn1a-Tg3-Cre_Ai14_IVSCC_-177300.01.02.01_473845048_m.swc" != i]
            iterswc=iter(swclist2)
            self.generate_morphology(cell, iterswc.next())
