...
if not os.path.isfile('/home/zaza3/Downloads/trunk/examples/patdemo/cells/j4a.hoc') and RANK==0:
    #get the model files:
...
cell_parameters = {          # various cell parameters,
    'morphology' : '/home/zaza3/Downloads/trunk/examples/patdemo/cells/j4a.hoc', # Mainen&Sejnowski, 1996
...
for i in range(RANK, int(len(allrows)), SIZE):
    if((i<n_cell) and (0<i)):
        ...
        if(int(len(s))>9):#//This condition is counter intuitive many cells are  
            ...
            print storename
            if(fn.match(storename)):
                print storename
                h.cell=h.mkcell(storename)
                h('cells.append(cell)')
                pc.set_gid2node(int(cnt), int(pc.id)) #// associate gid i with this host
                nc=cell.connect2target(nil)
                pc.cell(cnt, nc)
                gidvec.append(cnt)
                cnt+=pc.nhost  
                h('forsec cell.all{ psection() }')

    #print i 
cells = h.cells
allsectionsinpython = h.allsec()
#h('forsec cells.o(1).all{ psection() }')
"""
...
#cells=0
#cells=[cells]*40
cells=[ LFPy.Cell(**cell_parameters) for i in range(int(n_cell))]
    ...
    if((cnt<n_cell-1) and (0<i)):#While there are less than 100 cells on every node.
        ...
        if(int(len(s))>9):#//This condition is counter intuitive many cells are         
            ...
            if(re.search('.swc',storename)):
                ...
                cells[i]= LFPy.Cell(**cell_parameters)
                cells[i].set_rotation(x=4.99, y=-4.33, z=z_rotation[RANK])
                cells[i].set_pos(xpos=x_cell_pos[cnt])
                #cells[i]=cell
                #cells.append(cell)
zips = []
i=0
for i in xrange(2,int(len(cells)-1)):
    print i
    #print cells[i]
    #if(i>0):
    for x, z in cells[i].get_idx_polygons():
...
quit()    
                ...
                #h('cell.pyr()')
                #h('cells.append(cell)')
                #h('p.set_gid2node(int(cnt1)'), int(pc.id)) #// associate gid i with this host
                #nc=cell.connect2target(nil)
                #pc.cell(cnt, nc)
                #gidvec.append(cnt1)
                #cnt1+=pc.nhost  
...
#print cells[:]#.get_idx_polygons()
#print cells[0].get_idx_polygons()
#cells = h.cells
#allsectionsinpython = h.allsec()
...
#This is where
#execfile('/home/zaza3/Downloads/trunk/examples/expericomp20140421/mycode2.py')

#for i=RANK; i < py.int(py.len(py.allrows)) ; i += SIZE:   
    ...
    if((i<n_cell) and (0<i)):
        ...
        if(int(len(s))>9):#//This condition is counter intuitive many cells are  
            ...
            if(re.search('.swc',storename)):
                ...
                h('cell.pyr()')
                h('cells.append(cell)')
                #pc.set_gid2node(int(cnt1), int(pc.id)) #// associate gid i with this host
                #nc=cell.connect2target(nil)
                #pc.cell(cnt, nc)
                #gidvec.append(cnt1)
                #cnt1+=pc.nhost  
