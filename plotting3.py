  ...
  for j in xrange(0,len(trains)):
    if(k!=j):
      ...
      if((sum(trains[j])!=0)|(sum(trains[k])!=0)):
        ...
        print sent.H," ",pspke[j]," ",pspke[i]," ",j, k 
...
#Below, remove Not A Numbers (NANs)
...
#coh = C.coherence[idx1, idx2].reshape(4, 4, entfa.shape[0])
 ...
 for j in xrange(0,len(msgcv)):
   if (int(h.prunenet>0)):
     ...
     if entfa[i][j]>(3*np.std(entfa)):
       dirfinals2.add_edge(i,j,weight=entfa[i][j]) 
...
#thent.add_edge(int(sources),int(targets),weight=ent)
