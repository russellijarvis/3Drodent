        ...
        if (data!=None)and(RANK!=s):
            for sec in h.spk_rx_ls:#iterate through post synaptic locations.
                ...
                #print 'code hangs here'
                    ...
                    r=float(r)
                    print ' r ', r
                    if(r<50):#I still need to debug the code, to find out why it won't search the finest levels of granularity.
                        gidcompare=''
                        '''
                        It never returns from this.
                        h('sec_string2=py.sec.name()')
                        h('strobj.head(sec_string2, "].", cell_here)') 
                        #h('strobj.head(sec_string1, "].", pre_src)') 
                        #h('strobj.tail(pre_src,"Cell",tail_src)')
                        h('strobj.tail(cell_here,"Cell",tail_target)')	
                        h('strobj.right(tail_target, 1)')
                        #h('strobj.right(tail_src, 1)')
                        h('py.gidcompare=Cell[py.int(tail_target)].gid1')
                        '''
                ...
                #secnames=data[1][0][:]
                        ...
                        h(post_syn)
                        h('syn_.cid=gidn') #This makes the gid a property of the synapse. 
