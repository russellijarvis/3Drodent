        ...
        if (data!=None):#and(RANK!=s):
            for i in xrange(0,NCELL):
                ...
                #print 'code hangs here'
                                ...
                                r=float(r)
                                print ' r ', r
                                if(r<35):#I still need to debug the code, to find out why it won't search the finest levels of granularity.
                                    gidcompare='' 
...
if RANK==0:
  ...
  #for i in xrange(0,len(sconm[0][:][0])):
...
#print len(sconm[0][:][0]), 'the length'
  
set_pointers="""proc setpointers() {local x //local done  
    forall{      
	if (ismembrane("xtra")) { 
     	    for (x, 0) {  
   		
    		setpointer im_xtra(x), i_membrane(x) 
		setpointer ex2_xtra(x), e_extracellular(x)  
