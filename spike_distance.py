...
fig.clf()
plt.hold(True)
for i in xrange(0, int(len(allmemb))):
    tc = np.array(allmemb[int(i)])
    #tc = tc + i * 80
...
maximum_time=0
times=[]
for i in xrange(0,2308):
    print i, np.shape(x)
    indexs=np.where(x[:,1]==i)
    times=x[indexs,0]
    if np.sum(x[indexs,0])!=0:
        if np.max(x[indexs,0])>maximum_time:
            maximum_time=np.max(x[indexs,0])
            print times
        list_spike_trains.append(times)
